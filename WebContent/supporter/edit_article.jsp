...
<body>
                        ...
                        <div class="mail-body">
                            ...
                            <form class="form-horizontal" method="get">
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                        <input id="aname" type="text" class="form-control" value="${article.aname }">
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control" id="cid">
                                        	...
                                        	<c:if test="${article.cid!='0' }">
                                        		<c:forEach items="${category }" var="citem">
                                        			<c:if test="${article.cid==citem.cid }">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                        <input id="atags" type="text" class="form-control" value="${article.atags }">
                                    </div>
                                </div>
                            </form>
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-footer">
			        ...
			        <button id="success" type="button" class="btn btn-primary" data-dismiss="modal" onclick="window.location.reload(true)">前往查看</button>
			      </div>
			    </div><!-- /.modal-content -->
			  </div><!-- /.modal-dialog -->
			</div><!-- /.modal -->
    ...
    <script>
    	...
    	$(function(){
    		...
    		timer = setInterval(function(){
            	if (ue.hasContents()) {
            		...
            		if ("${type }"==="old") {
            			//因为article的acontent是多行字符串,要采用特殊方式处理
            ...
            //点击放弃按钮
    		 ...
    		 $("#send").click(function(){
    			var target = "";
    			if ("${type }" === "new") {
    				target = "${pageContext.request.contextPath }/admin/newArticle";
    			} else {
    				target = "${pageContext.request.contextPath }/admin/updateArticle";
    			}
    			var param = getParameters();
              	$.ajax({
              		...
              		success:function(data) {
              			if (data==="true") {
              				$("#success").attr("onclick","redirect('${pageContext.request.contextPath}/readArticle?aid=${article.aid }')");
                  			showTips("发表成功！");
                  		} else {
    		 ...
    		 $(".draft").click(function(){
    			//获取需要存入草稿箱的参数
    			 var dname = encodeURI(encodeURI($("#aname").val()));
    			 var dcontent = encodeURI(encodeURI(ue.getContent()));
    			 var dtags = encodeURI(encodeURI($("#atags").val()));
    			 var cid = $("select").val();
    			 var aid = "${article.aid }";
    			 var atype = "${type }";
    			 var param = {"dname":dname,"dcontent":dcontent,"dtags":dtags,"cid":cid,"aid":aid,"type":atype };
    			 $.ajax({
    				 type:"post",
    				 url:"${pageContext.request.contextPath }/admin/newDraft",
    				 data:param,
    				 success:function(data){
    					if (data==="true") {
    	...
    	function getParameters() {
    		var aid = "${article.aid }";
          	var aname = encodeURI(encodeURI($("#aname").val()));
          	var acontent = encodeURI(encodeURI(ue.getContent()));
          	//旧的文章重新发表是不需要更改日期的
          	var adate = "${article.adate }";
          	if ("${type }" === "new")
          		adate = encodeURI(encodeURI(new Date().toLocaleString()));
          	var aview = "${article.aview }";
          	var acomment = "${article.acomment }";
          	var atags = encodeURI(encodeURI($("#atags").val()));
          	var summary = encodeURI(encodeURI(getSummary()));
          	var cid = $("select").val();
          	var param = {"aid":aid,"aname":aname,"acontent":acontent,"adate":adate,"aview":aview,"acomment":acomment,"atags":atags,"summary":summary,"cid":cid};
          	return param;
    	}
    	//从内容摘取摘要
       function getSummary() {
